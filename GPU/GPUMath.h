...
// Assembly directives replacement for HIP/Generic C++
// Using unsigned __int128 for 64-bit carry arithmetic.
// Each function using these macros must declare: uint64_t _carry = 0;

#define UADDO(c, a, b)                                                         \
  {                                                                            \
    unsigned __int128 _r = (unsigned __int128)(a) + (b);                       \
    (c) = (uint64_t)_r;                                                        \
    _carry = (_r >> 64);                                                       \
  }
#define UADDC(c, a, b)                                                         \
  {                                                                            \
    unsigned __int128 _r = (unsigned __int128)(a) + (b) + _carry;              \
    (c) = (uint64_t)_r;                                                        \
    _carry = (_r >> 64);                                                       \
  }
#define UADD(c, a, b)                                                          \
...
#define UADDO1(c, a)                                                           \
  {                                                                            \
    unsigned __int128 _r = (unsigned __int128)(c) + (a);                       \
    (c) = (uint64_t)_r;                                                        \
    _carry = (_r >> 64);                                                       \
  }
#define UADDC1(c, a)                                                           \
  {                                                                            \
    unsigned __int128 _r = (unsigned __int128)(c) + (a) + _carry;              \
    (c) = (uint64_t)_r;                                                        \
    _carry = (_r >> 64);                                                       \
  }
#define UADD1(c, a)                                                            \
...
// MADDO: r = a*b + c (hi part? No, mad.hi.cc.u64 is (a*b)hi + c ... wait)
// NVCC: mad.hi.cc.u64 d, a, b, c; -> d = high(a*b) + c. CC update.
#define MADDO(r, a, b, c)                                                      \
  {                                                                            \
    unsigned __int128 _prod = (unsigned __int128)(a) * (b);                    \
    uint64_t _hi = (_prod >> 64);                                              \
    unsigned __int128 _sum = (unsigned __int128)_hi + (c);                     \
    (r) = (uint64_t)_sum;                                                      \
    _carry = (_sum >> 64);                                                     \
  }
#define MADDC(r, a, b, c)                                                      \
  {                                                                            \
    unsigned __int128 _prod = (unsigned __int128)(a) * (b);                    \
    uint64_t _hi = (_prod >> 64);                                              \
    unsigned __int128 _sum = (unsigned __int128)_hi + (c) + _carry;            \
    (r) = (uint64_t)_sum;                                                      \
    _carry = (_sum >> 64);                                                     \
  }
#define MADD(r, a, b, c)                                                       \
...
#define UMultSpecial(r, a)                                                     \
  {                                                                            \
    ...
    r[0] = (a[0] << 32) + (a[0] << 9) + (a[0] << 8) + (a[0] << 7) +            \
           (a[0] << 6) + (a[0] << 4) + a[0];                                   \
    r[1] = (a[1] << 32) + (a[1] << 9) + (a[1] << 8) + (a[1] << 7) +            \
           (a[1] << 6) + (a[1] << 4) + a[1];                                   \
    MADDO(r[1], a[0], 0x1000003D1ULL, r[1]);                                   \
    r[2] = (a[2] << 32) + (a[2] << 9) + (a[2] << 8) + (a[2] << 7) +            \
           (a[2] << 6) + (a[2] << 4) + a[2];                                   \
    MADDC(r[2], a[1], 0x1000003D1ULL, r[2]);                                   \
    r[3] = (a[3] << 32) + (a[3] << 9) + (a[3] << 8) + (a[3] << 7) +            \
           (a[3] << 6) + (a[3] << 4) + a[3];                                   \
    temp = r[3];                                                               \
...
__device__ void
ModSub256isOdd(uint64_t *a, uint64_t *b,
               uint8_t *parity) { // no need to compute py, we need only parity
  uint64_t _carry = 0;
  uint64_t t;
  uint64_t T[4];
...
#define MADDS(r, a, b, c)                                                      \
  {                                                                            \
    __int128 _prod = (__int128)(int64_t)(a) * (__int128)(int64_t)(b);          \
    int64_t _hi = (_prod >> 64);                                               \
    unsigned __int128 _sum = (unsigned __int128)_hi + (c) + _carry;            \
    (r) = (uint64_t)_sum;                                                      \
  }
...
__device__ void _DivStep62(uint64_t u[5], uint64_t v[5], int32_t *pos,
...
__device__ void MatrixVecMul(uint64_t u[5], uint64_t v[5], int64_t _11,
                             int64_t _12, int64_t _21, int64_t _22) {
  uint64_t _carry = 0;
...
__device__ __noinline__ void _ModInv(uint64_t *R) {
  ...
  // Compute modular inverse of R mop P (using
  // 320bits signed integer) 0 < this < P  , P
  // must be odd Return 0 if no inverse See
  // IntMod.cpp for more info.
